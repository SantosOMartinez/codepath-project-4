...
class HuntDetailViewController: UIViewController {
    ...
    @IBOutlet weak var emptyMapLabel: UIStackView!
    ...
    override func viewDidLoad() {
        ...
        emptyMapLabel.layer.opacity = 0.5
    ...
    private func updateUI() {
        ...
        let completedImage = UIImage(systemName: hunt.isComplete ? "checkmark.circle" : "circle")
        ...
        completedLabel.text = hunt.isComplete ? "Complete" : "Incomplete"

        let color: UIColor = hunt.isComplete ? UIColor(named: "AccentColor")! : UIColor(named: "border")!
        completedImageView.tintColor = color
        completedLabel.textColor = color

        mapView.isHidden = !hunt.isComplete
        resetButton.isHidden = !hunt.isComplete
        attachPhotoButton.isHidden = hunt.isComplete
    }
    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        ...
        actionSheet.addAction(UIAlertAction(title: "Camera", style: .default, handler: {(action: UIAlertAction) in
            self.authCameraPicker()
        }))

        actionSheet.addAction(UIAlertAction(title: "Photo Library", style: .default, handler: {(action: UIAlertAction) in
                // Show photo picker
            self.authImagePicker()
        }))
    ...
    private func presentImagePicker(_ stauts: PHAuthorizationStatus = .authorized) {
    ...
    func updateMapView() {
        emptyMapLabel.isHidden = hunt.isComplete
        resetButton.isHidden = !hunt.isComplete
...
extension HuntDetailViewController: PHPickerViewControllerDelegate {
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        guard let assetId = result?.assetIdentifier,
            ...
            DispatchQueue.main.async {
                let newAlert = UIAlertController(title: "Location not found", message: "Was not able to identify a location on the selected photo.", preferredStyle: .alert)
    ...
    func presentGoToSettingsAlert() {
        let alertController = UIAlertController (
            ...
            message: "In order to post a photo to complete a task, we need access. You can allow access in Settings",
...
extension HuntDetailViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
